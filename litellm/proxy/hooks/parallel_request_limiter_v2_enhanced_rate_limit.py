...
class _PROXY_MaxParallelRequestsHandler_v2_EnhancedRateLimit(BaseRoutingStrategy, CustomLogger):
    ...
    def _get_current_usage_key_per_hour(
        self,
        user_api_key_dict: UserAPIKeyAuth,
        precise_hour: str,
        model: Optional[str],
        rate_limit_type: Literal["key", "model_per_key", "user", "customer", "team"],
        group: RateLimitGroupsPerHour,
    ) -> Optional[str]:
        if rate_limit_type == "model_per_key":
            return f"{self.prefix}::{user_api_key_dict.api_key}::{model}::{precise_hour}::{group}"
        else:
            return None
    ...
    async def check_key_in_limits_v2_per_hour(
        self,
        user_api_key_dict: UserAPIKeyAuth,
        data: dict,
        precise_hour: str,
        ...
        # Add RPH key separately, using precise_hour
        rph_key = self._get_current_usage_key_per_hour(
            user_api_key_dict=user_api_key_dict,
            precise_hour=precise_hour,
            model=data.get("model", None),
            rate_limit_type=rate_limit_type,
            group=cast(RateLimitGroupsPerHour, "rph"),
        )
        if rph_key is not None:
            increment_list.append((rph_key, increment_value_by_group["rph"]))
            decrement_list.append((rph_key, -1 if increment_value_by_group["rph"] == 1 else 0))
            slot_cache_keys.append(rph_key)
        ...
        if rph_key in key_to_value_map:
            value = key_to_value_map[rph_key]
            if value is not None:
                total_rph += int(value)

        should_raise_error = False
        if rph_limit is not None:
            should_raise_error = should_raise_error or total_rph > rph_limit
        ...
        if should_raise_error:
            ...
            additional_details=f"{CommonProxyErrors.max_parallel_request_limit_reached.value}. Hit limit for {rate_limit_type}. Current usage: current_rph: {total_rph}. Current limits: rph_limit: {rph_limit}."
            if litellm.enable_lazy_rate_limit_exception_for_parallel_request_limiter:
                data.setdefault("metadata", {})
                data["metadata"]["lazy_rate_limit_exception_for_parallel_request_limiter"] = additional_details
                return
    ...
    async def async_pre_call_hook(  # noqa: PLR0915
        ...
        rph_limit = getattr(user_api_key_dict, "rph_limit", sys.maxsize)
        if rph_limit is None:
            rph_limit = sys.maxsize
        rpd_limit = getattr(user_api_key_dict, "rpd_limit", sys.maxsize)
        if rpd_limit is None:
            rpd_limit = sys.maxsize
        ...
        precise_hour = f"{current_date}-{current_hour}"
        ...
        if requested_model and (
            ...
            if should_check_rate_limit:
                ...
                tasks.append(
                    self.check_key_in_limits_v2_per_hour(
                        user_api_key_dict=user_api_key_dict,
                        data=data,
                        precise_hour=precise_hour,
                        rph_limit=model_specific_rph_limit,
                        rate_limit_type="model_per_key",
                    )
                )
    ...
    async def _update_usage_in_cache_post_call_per_hour(
        self,
        user_api_key_dict: UserAPIKeyAuth,
        precise_hour: str,
        model: Optional[str],
        ...
        for rate_limit_type in rate_limit_types:
            for group in ["rph"]:
                key = self._get_current_usage_key_per_hour(
                    user_api_key_dict=user_api_key_dict,
                    precise_hour=precise_hour,
                    model=model,
                    rate_limit_type=cast(RateLimitTypes, rate_limit_type),
                    group=cast(RateLimitGroupsPerHour, group),
                )
                if key is None:
                    continue
                increment_list.append((key, increment_value_by_group[group]))
    ...
    async def async_log_success_event(  # noqa: PLR0915
        ...
        try:
            ...
            current_minute = datetime.now().strftime("%M")
            precise_hour = f"{current_date}-{current_hour}"
            precise_minute = f"{precise_hour}-{current_minute}"
            ...
            await self._update_usage_in_cache_post_call_per_hour(
                user_api_key_dict=UserAPIKeyAuth(
                    api_key=user_api_key,
                    user_id=user_api_key_user_id,
                    team_id=user_api_key_team_id,
